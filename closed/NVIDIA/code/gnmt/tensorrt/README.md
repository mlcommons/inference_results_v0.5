...
This benchmark performs translation from one natural language to another, such as English to German. As explained below, it implements Google's Neural Machine Translation (GNMT) system.
...
*  CgPersistentLSTMPlugin_TRT: for small batch sizes on the encoder side
*  SingleStepLSTM_TRT: for the decoder running on Xavier
*  GNMTDecoderPlugin: implements GNMT decoder running in INT8 precision on the other systems
...
The first two plugins are available in [TensorRT 6](https://developer.nvidia.com/tensorrt) release. The source codes of the other plugins can be found in [src/plugin](src/plugin).
...
#### Int8 Precision
In addition to running the GNMT network in FP16 precision, parts of the network, the decoder and the scorer, also support Int8 precision. This can be activated using the following argument:
```
