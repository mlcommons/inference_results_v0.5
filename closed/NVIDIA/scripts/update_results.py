...
from code.common import run_command, system_list
...
def main():
    ...
    for system_id,_,_ in system_list:
        for benchmark in ["resnet", "mobilenet", "ssd-large", "ssd-small", "gnmt"]:
            for scenario in ["SingleStream", "MultiStream", "Offline", "Server"]:
                print(">>>>>>>> Processing {:}-{:}-{:} <<<<<<<<".format(system_id, benchmark, scenario))
                input_glob = os.path.join(args.input_dir, "*", system_id, benchmark, scenario, "mlperf_log_accuracy.json")
                input_list = glob.glob(input_glob)

                if len(input_list) == 0:
                    print("Skipping because LoadGen logs do not exist.")
                    continue
