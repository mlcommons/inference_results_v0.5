...
class GNMTWrapper:
    ...
    def __init__(self, ckpt_path=None, hparams_path=None, vocab_prefix=None, outdir=None, batch_size=6):
    ...
    def translate(self, sentence_id_list):
        ...
        with self.infer_model.graph.as_default():
            ...
            self.sess.run(
                ...
                feed_dict={
                    # FIXME (bryce): Uncomment this line if you want a shorter sentence_id_list for testing
                    # self.infer_model.src_placeholder: [self.infer_data[i] for i in sentence_id_list[0:6]],
...
class GNMTRunner (Runner):
    ...
    def process(self, qitem):
        ...
        for i in range(0, num_samples, bs):
            #Kobrien: Uncomment following line if you want to see progress printed during the run
            #print('Running sample = ' + str(i) + '-' + str(num_samples))
...
if __name__ == "__main__":
    ...
    # FIXME (bryce): Change this default to run either Performance or Accuracy
    ...
    settings.performance_sample_count_override = 3903900
    settings.qsl_rng_seed = 3133965575612453542
    settings.sample_index_rng_seed = 665484352860916858
    settings.schedule_rng_seed = 3622009729038561421

    #min_samples_per_query needs to be 24576
    try:
