...
class BaseModel(object):
  ...
  def infer(self, sess):
    ...
    output_tuple = InferOutputTuple(infer_logits=sess.graph.get_operation_by_name(self.infer_summary.name),
                                    infer_summary=sess.graph.get_operation_by_name(self.infer_summary.name),
                                    sample_id=sess.graph.get_operation_by_name(self.infer_summary.name),
                                    sample_words=sess.graph.get_tensor_by_name(self.sample_words.name))

    # FIXME (bryce): Use this code to debug average output inference time
