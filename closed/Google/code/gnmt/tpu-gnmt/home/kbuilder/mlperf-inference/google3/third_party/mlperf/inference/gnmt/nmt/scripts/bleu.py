...
class RunningBLEUScorer:  # pylint: disable=old-style-class
  """Class to compute running BLEU scores."""
  ...
  def add_sentence_with_multiple_refs(self, references, translation):
    """Add a single reference, with potentially multiple references."""
    ...
    new_matches_by_order, new_possible_matches_by_order = self._get_ngram_match_values(
  ...
  def calc_bleu_score(self):
    ...
    return (bleu, precisions, bp, ratio, self.translation_length,
            self.reference_length)

  def _get_ngram_match_values(self, ref_ngram_counts, translation_ngram_counts,
                              translation_length):
    """Compute matching percentages for different order ngrams."""
...
def compute_bleu(reference_corpus, translation_corpus, max_order=4,
  ...
  Args:
    ...
    translation_corpus: list of translated sentences to score. Each sentence
      should be tokenized into a list of tokens. translation_corpus[i][j]
      represents the j'th word for the i'th sentence
  ...
  """
  running_bleu = RunningBLEUScorer(max_order=max_order, smooth=smooth)

  for (references, translation) in zip(reference_corpus,
                                       translation_corpus):
    running_bleu.add_sentence_with_multiple_refs(references, translation)

  return running_bleu.calc_bleu_score()
