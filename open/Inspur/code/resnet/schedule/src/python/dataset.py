...
def pre_process_vgg(img, dims=None, need_transpose=False):
    ...
    cv2_interpol = cv2.INTER_AREA
    #img = resize_with_aspectratio(img, 256, 256, inter_pol=cv2_interpol)
    img = resize_with_aspectratio(img, 256, 256, scale=100, inter_pol=cv2_interpol)
